...
class Docker(object):
    ...
    def push(self, repository, tag):
        ...
        """
        push_registroy = "{}/{}".format(Docker.REGISTRY, repository)

        log.info("Pushing {repository}:{tag}".format(
            repository=repository, tag=tag))
        output = self.api_client.push(repository=push_registroy, tag=tag,
    ...
    for name in builds:
        ...
        for target in targets:
            ...
            dockerpy.build(context=context,
                           ...
                           labels=labels)
            if push:
                dockerpy.push(reposotory=repository, tag=latest_tag)
